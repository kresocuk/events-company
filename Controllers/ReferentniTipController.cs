        ...
        public IActionResult Create(ReferentniTip tip) {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ctx.Add(tip);
                    ctx.SaveChanges();
                    TempData[Constants.Message] = $"ReferentniTip {tip.Naziv} uspješno dodan.";
                ...
                {
                    logger.LogError($"Pogreška prilikom dodavanja novog ref tipa {e.CompleteExceptionMessage()}");
        ...
        public async Task<IActionResult> Update(int Id, int page = 1, int sort = 1, bool ascending = true) {
            try {
                ...
                if (ok)
                {
                    try
                    {
                        TempData[Constants.Message] = $"ReferentniTip {tip.Naziv} uspješno azuriran.";
                        TempData[Constants.ErrorOccurred] = false;
        ...
        public IActionResult Delete(int Id, int page=1, int sort=1, bool ascending=true)
        {
            ...
            else
            {
                try
                {
                    ...
                    ctx.Remove(tip);
                    ctx.SaveChanges();
                    TempData[Constants.Message] = $"ReferentniTip {tip.Naziv} uspješno obrisan.";
                    TempData[Constants.ErrorOccurred] = false;
                }
                catch (Exception e)
                {
